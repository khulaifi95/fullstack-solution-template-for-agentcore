...
SYSTEM_PROMPT = (
    ...
    "datasets.\n\n"
    "Use `search_knowledge_base` for questions answerable from policies, SOPs, "
    "emails, or reports. Use "
    ...
    "data, then combine the answer.\n\n"
    "CITATIONS ARE MANDATORY. Whenever you use information from ANY tool result "
    "— document passages OR structured data — cite its `source` inline as a "
    "markdown link using the `link` field (e.g. \"([Contractor listing.xlsx](<link>))\" "
    "or \"([SOP-CO-003.pdf](<link>))\"), and END every such response with a "
    "'References' section listing each cited source as a clickable markdown link. "
    "This applies equally to structured-data answers and document answers. If a "
    "tool returns no relevant results, say so rather than guessing.\n\n"
